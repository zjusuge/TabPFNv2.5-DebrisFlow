...
def plot_model_comparison(r2_df, wilcoxon_df, set_label, fig_name,
    ...
    ax.text(panel_a_info_pos[0], panel_a_info_pos[1],
            f"{set_label}\nTabPFN mean R² = {tabpfn_mean:.4f}",
            transform=ax.transAxes, fontsize=12,
            va=panel_a_info_va, ha=panel_a_info_ha,  # ★ Configurable horizontal alignment
            bbox=dict(boxstyle="round,pad=0.3", fc="#FFF0E8",
                      ec="#D55E00", lw=0.8))
    ...
    for i, row in wdf_c.iterrows():
        x_txt = min(row["effect_r"] + 0.03, 0.92)
        ax.text(x_txt, i, str(row["sig"]).strip(),
                va="center", fontsize=12.5,
                fontweight="bold", color="#333333")

    leg_handles = [Patch(fc=SC[k], ec="#888888", lw=0.5,
                         label=fr"$p_{{\mathrm{{Holm}}}}$ {lab}")
                   for k, lab in [("***", "< 0.001"), ("**", "< 0.01"),
                                  ("*", "< 0.05"), ("n.s.", "≥ 0.05")]]
    ax.legend(handles=leg_handles, fontsize=11,
    ...
    ax.legend(fontsize=11.5, loc=panel_d_legend_loc,
              framealpha=0.95, edgecolor="lightgray")

    pct = wins_t / n_min * 100
    ax.text(0.97, 0.03,
            f"TabPFN superior in {wins_t}/{n_min} folds ({pct:.0f}%)",
