...
# ═══════════════════════════════════════════════════════════════
# Fixed first repetition for the sample and fan-chart displays.
...
alphas_c  = sorted(df_s[COL_ALPHA].unique(), reverse=True)
labels_c  = [f"{(1-a)*100:.0f}%" for a in alphas_c]

width_dists = []
for a in alphas_c:
    w = df_s.loc[np.abs(df_s[COL_ALPHA] - a) < 1e-6, "xc_wid"].values
...
ax.fill_between(x_diag, x_diag - 0.05, x_diag + 0.05,
                ...
                label="Reference band: ±5 pp", zorder=0)
ax.plot(x_diag, x_diag, "--", color="0.55", lw=1.2,
        label="Ideal calibration", zorder=1)

ax.plot(coverages_b, empirical_b, 'o-', color=C_BLUE, ms=7, lw=2,
...
ax.legend(loc="upper center", frameon=True, framealpha=0.95,
          fontsize=12, edgecolor="0.80", bbox_to_anchor=(.5,-.20), ncol=2)
...
ax.set_ylabel(r"Interval width (log$_{10}$ units)",
              fontweight="normal")

ax.set_title(f'90% median width = {np.median(width_dists[labels_c.index("90%")]):.3f}', fontsize=14)
...
y_labs = [f'No. {sid+1}' for sid in PICK_IDS]
...
ax.set_ylabel("Selected catchment", fontweight="normal")
...
ax.legend(handles=legend_handles, loc="upper center",
          ...
          fontsize=12, ncol=4, edgecolor="0.80",     # ★ 10 → 12
